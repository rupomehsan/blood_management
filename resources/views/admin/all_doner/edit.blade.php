...
<div class=" register view-doner" style="padding-top: 7%;margin-left: 17%;">
    <div class="row container">
        ...
        <button><a href="all-doner.php">ALL BLOOD DONER</a></button>
         <h4><i class="fa fa-angle-right"></i>UPDATE BLOOD DONER</h4><hr><div class="col-md-6">
...
<form id="donerupdateForm" enctype="multipart/form-data">
    ...
    <div class="form-group">
        ...
        <input type="text" minlength="11" maxlength="11" id="phone" name="phone" class="form-control" value="{{$editdoner->phone}}">
    </div>
    <div class="form-group">
        <h5 style="padding-bottom: 10px;">Address: <span style="color:red;">*</span></h5>
        <input type="text" class="form-control" id="address" name="address" value="{{$editdoner->address}}">
    </div>

    <h5 style="padding-bottom: 10px;">social <span style="color:red;">*</span></h5>
    <div class="form-group">
    <input type="text" class="form-control" id="social_link" name="social" value="{{$editdoner->social_link}}">
    </div>
    <div class="form-group">
        ...
        <select class="form-control" id="bloodgp_id" >  
            ...
            <option {{$editdoner->bloodgroup_id == $bloodgp->id ?'selected':''}} value="{{$bloodgp->id}}">{{$bloodgp->name}}</option>     
            @endforeach                 
        </select>
    </div>
    ...
    <select class="form-control" id="divition_id" >  
        ...
        <option {{$editdoner->divition_id == $divition->id? 'selected' :''}} value="{{$divition->id}}">{{$divition->name}}</option>
        @endforeach
        </select>
    ...
    <div class="form-group">
        ...
        <select class="form-control" id="district_id" >  
            ...
            <option {{$editdoner->district_id == $district->id? 'selected' :''}} value="{{$district->id}}">{{$district->name}}</option>
            @endforeach
            </select>
    ...
    <div class="form-group" style="padding-left: 10px;border:1px solid black;">
            ...
            <div class="maxl">
                <div class="col-md-6">
                    <label class="radio inline">
                        <input {{$editdoner->donate_status === 'YES' ? 'checked':''}}  type="radio" name="status" value="YES">
                        <span> YES </span>
                    </label>
                </div>
                <div class="col-md-6">
                <label class="radio inline">
                    <input {{$editdoner->donate_status === 'NO' ? 'checked':''}} type="radio" name="status" value="NO">
                    <span>NO </span>
                </label>
                </div>
            </div>
        ...
        <h5>Gender : - </h5>
            <div class="maxl">
                <div class="col-md-6">
                    <label class="radio inline">
                        <input {{$editdoner->gender == 'Male' ? 'checked':''}} type="radio" name="gender" value="Male">
                        <span> Male </span>
                    </label>
                </div>
                <div class="col-md-6">
                <label class="radio inline">
                    <input  {{$editdoner->gender == 'Female' ? 'checked':''}} type="radio" name="gender" value="Female">
                    <span>Female </span>
                </label>
                </div>
            </div>
        ...
        <input type="file" class="custom-file-input" id="image" name="image" value="{{$editdoner->image}}">
